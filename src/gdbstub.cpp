...
vector<tuple<string, uint32_t, char,
             optional<vector<tuple<string, uint8_t, uint8_t>>>>>
    xml_data{
        {"zero", 0, 'g', nullopt},
        {"ra", 1, 'g', nullopt},
        {"sp", 2, 'g', nullopt},
...
uint32_t GDB_send(string buffer, uint32_t flags = 0) {
  ...
  while (total < buffer.size()) {
    ssize_t sent =
        send(gdb_recv, buffer.c_str() + total, buffer.size() - total, flags);
    if (sent <= 0)
...
void GDB_parsePacket(const char *buffer) {
  if (buffer[0] == '$') {
    ...
    if (packet.starts_with("qXfer:features:read:target.xml:")) {
      ...
      GDB_sendPacket(((startp + endp) >= gdb_xml.size() ? "l" : "m") +
                     gdb_xml.substr(startp, endp));
      return;
    }
...
// fpu
    ...
    if (packet.starts_with("M")) {
      ...
      uint64_t size =
          stoul(packet.substr(packet.find(',') + 1,
    ...
    if (packet.starts_with("m")) {
      ...
      for (uint64_t i = 0; i < size; i++) {
        optional<uint64_t> val =
            gdb_hart->mmio->load_GDB(gdb_hart, address + i, 8);
