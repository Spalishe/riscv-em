...
int main(int argc, char* argv[]) {
	if(argc > 1) {
		...
		if (dtb_has) {
			...
			// memmap.add_region(0x2200000, 0x10000);
			// DTB* dtb = new DTB(0x2200000,0x10000,hart->dram);
			// mmio->add(dtb);

			cpu_readfile(hart, dtb_path, 0x80000000, false);			
			dtb_path_in_memory = 0x80000000;
		} else {
			fdt = fdt_node_create(NULL);
			fdt_node_add_prop_u32(fdt, "#address-cells", 2);
			fdt_node_add_prop_u32(fdt, "#size-cells", 2);
			fdt_node_add_prop_str(fdt, "model", "Minimal RISC-V Machine");
		...
		CLINT* clint = new CLINT(0x02000000,hart->dram,2,(dtb_has ? NULL : fdt),1);
		...
		if(!dtb_has) {
			...
			size_t size = fdt_serialize(fdt,buffer,4096,0);
			...
			for (size_t i = 0; i < dtb_size; ++i) {
				dram_store(&hart->dram,0x80000000+i,8,bytes[i]);
			}	
			dtb_path_in_memory = 0x80000000;
		...
		if (!file.empty()) {
			cpu_readfile(hart, file, 0,false);
		}
