...
void exec_ADDW(HART *hart, inst_data inst) {
	hart->regs[inst.rd] = (int32_t) ((uint32_t)hart->regs[inst.rs1] + (uint32_t)hart->regs[inst.rs2]);
}
void exec_SUBW(HART *hart, inst_data inst) {
	hart->regs[inst.rd] = (int32_t) ((uint32_t)hart->regs[inst.rs1] - (uint32_t)hart->regs[inst.rs2]);
}
void exec_SLLW(HART *hart, inst_data inst) {
	hart->regs[inst.rd] = (int32_t) ((uint32_t)hart->regs[inst.rs1] << ((uint32_t)hart->regs[inst.rs2] & 0x1F));
}
void exec_SRLW(HART *hart, inst_data inst) {
    hart->regs[inst.rd] = (int32_t) ((uint32_t)hart->regs[inst.rs1] >> ((uint32_t)hart->regs[inst.rs2] & 0x1F));
}
void exec_SRAW(HART *hart, inst_data inst) {
    hart->regs[inst.rd] = (int32_t) (((int32_t) hart->regs[inst.rs1]) >> ((uint32_t)hart->regs[inst.rs2] & 0x1F));
}

// I-Type
void exec_ADDIW(HART *hart, inst_data inst) {
    hart->regs[inst.rd] = (int32_t) ((uint32_t)hart->regs[inst.rs1] + inst.imm);
}
...
void exec_SRAIW(HART *hart, inst_data inst) {
    hart->regs[inst.rd] = ((int32_t)hart->regs[inst.rs1]) >> inst.imm;
}

void exec_LD(HART *hart, inst_data inst) {
    uint64_t addr = hart->regs[inst.rs1] + (int64_t) inst.imm;
    std::optional<uint64_t> val = hart->mmio->load(hart,addr,64);
    if(val.has_value()) {
        hart->regs[inst.rd] = *val;
    }
}
void exec_LWU(HART *hart, inst_data inst) {
    ...
    if(val.has_value()) {
        hart->regs[inst.rd] = (uint32_t) *val;
    }
}
...
void exec_SD(HART *hart, inst_data inst) {
    uint64_t addr = hart->regs[inst.rs1] + (int64_t) inst.imm;
    hart->mmio->store(hart,addr,64,hart->regs[inst.rs2]);
}
...
void exec_SLL(HART *hart, inst_data inst) {
	hart->regs[inst.rd] = hart->regs[inst.rs1] << (hart->regs[inst.rs2] & 0x3f);
}
void exec_SRL(HART *hart, inst_data inst) {
	hart->regs[inst.rd] = hart->regs[inst.rs1] >> (hart->regs[inst.rs2] & 0x3f);
}
void exec_SRA(HART *hart, inst_data inst) {
	hart->regs[inst.rd] = ((int64_t) hart->regs[inst.rs1]) >> (hart->regs[inst.rs2] & 0x3f);
}
void exec_SLT(HART *hart, inst_data inst) {
	hart->regs[inst.rd] = ((int64_t) hart->regs[inst.rs1] < (int64_t) hart->regs[inst.rs2]) ? 1 : 0;
}
...
void exec_ADDI(HART *hart, inst_data inst) {
    hart->regs[inst.rd] = hart->regs[inst.rs1] + (int64_t) inst.imm;
}
...
void exec_SRAI(HART *hart, inst_data inst) {
    hart->regs[inst.rd] = ((int64_t)hart->regs[inst.rs1]) >> inst.imm;
}
void exec_SLTI(HART *hart, inst_data inst) {
    hart->regs[inst.rd] = ((int64_t) hart->regs[inst.rs1] < (int64_t) inst.imm) ? 1 : 0;
}
...
void exec_LB(HART *hart, inst_data inst) {	
    ...
    if(val.has_value()) {
        hart->regs[inst.rd] = (uint64_t)(int8_t) *val;
    }
}
void exec_LH(HART *hart, inst_data inst) {
    ...
    if(val.has_value()) {
        hart->regs[inst.rd] = (uint64_t)(int16_t) *val;
    }
}
void exec_LW(HART *hart, inst_data inst) {
    ...
    std::optional<uint64_t> val = hart->mmio->load(hart,addr,32);
    if(val.has_value()) {
        hart->regs[inst.rd] = (uint64_t)(int32_t) *val;
    }
}
void exec_LBU(HART *hart, inst_data inst) {
    ...
    std::optional<uint64_t> val = hart->mmio->load(hart,addr,8);
    if(val.has_value()) {
        hart->regs[inst.rd] = (uint64_t)(uint8_t) *val;
    }
}
void exec_LHU(HART *hart, inst_data inst) {
    ...
    std::optional<uint64_t> val = hart->mmio->load(hart,addr,16);
    if(val.has_value()) {
        hart->regs[inst.rd] = (uint64_t)(uint16_t) *val;
    }
}
...
void exec_SB(HART *hart, inst_data inst) {
    uint64_t addr = hart->regs[inst.rs1] + (int64_t) inst.imm;
    hart->mmio->store(hart,addr,8,hart->regs[inst.rs2]);
}
void exec_SH(HART *hart, inst_data inst) {
    uint64_t addr = hart->regs[inst.rs1] + (int64_t) inst.imm;
    hart->mmio->store(hart,addr,16,hart->regs[inst.rs2]);
}
void exec_SW(HART *hart, inst_data inst) {
    uint64_t addr = hart->regs[inst.rs1] + (int64_t) inst.imm;
    hart->mmio->store(hart,addr,32,hart->regs[inst.rs2]);
}

// B-Type
void exec_BEQ(HART *hart, inst_data inst) {
    if ((int64_t) hart->regs[inst.rs1] == (int64_t) hart->regs[inst.rs2]) {
        if((hart->pc + (int64_t) inst.imm) % 4 != 0)
            hart->cpu_trap(EXC_INST_ADDR_MISALIGNED,hart->pc + (int64_t) inst.imm,false);
        else
...
void exec_BLTU(HART *hart, inst_data inst) {
    if (hart->regs[inst.rs1] < hart->regs[inst.rs2]) {
        if((hart->pc + (int64_t) inst.imm) % 4 != 0)
            hart->cpu_trap(EXC_INST_ADDR_MISALIGNED,hart->pc + (int64_t) inst.imm,false);
        else
            hart->pc = hart->pc + (int64_t) inst.imm - 4;
...
void exec_JALR(HART *hart, inst_data inst) {
    ...
    if((hart->regs[inst.rs1] + (int64_t) inst.imm) % 4 != 0)
        hart->cpu_trap(EXC_INST_ADDR_MISALIGNED,hart->regs[inst.rs1] + (int64_t) inst.imm,false);
    else
        hart->pc = hart->regs[inst.rs1] + (int64_t) inst.imm - 4;
	hart->regs[inst.rd] = tmp+4;
}

// what

void exec_LUI(HART *hart, inst_data inst) {
	hart->regs[inst.rd] = (int64_t) (int32_t) (inst.imm << 12);
}
