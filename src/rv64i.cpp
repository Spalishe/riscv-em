...
void exec_ADDW(struct HART *hart, uint32_t inst) {
	...
	print_d(hart,"{0x%.8X} [ADDW] rs1: %в; rs2: %d; rd: %d",hart->pc,rs1(inst),rs2(inst),rd(inst));
}
void exec_SUBW(struct HART *hart, uint32_t inst) {
	hart->regs[rd(inst)] = (int64_t) ((int32_t) hart->regs[rs1(inst)] - (int32_t) hart->regs[rs2(inst)]);
	print_d(hart,"{0x%.8X} [SUBW] rs1: %d; rs2: %d; rd: %d",hart->pc,rs1(inst),rs2(inst),rd(inst));
}
void exec_SLLW(struct HART *hart, uint32_t inst) {
	...
	print_d(hart,"{0x%.8X} [SLLW] rs1: %d; rs2: %d; rd: %d",hart->pc,rs1(inst),rs2(inst),rd(inst));
}
void exec_SRLW(struct HART *hart, uint32_t inst) {
	...
	print_d(hart,"{0x%.8X} [SRLW] rs1: %d; rs2: %d; rd: %d",hart->pc,rs1(inst),rs2(inst),rd(inst));
}
void exec_SRAW(struct HART *hart, uint32_t inst) {
	...
	print_d(hart,"{0x%.8X} [SRAW] rs1: %d; rs2: %d; rd: %d",hart->pc,rs1(inst),rs2(inst),rd(inst));
}

// I-Type
void exec_ADDIW(struct HART *hart, uint32_t inst) {
    hart->regs[rd(inst)] = (int64_t) ((int32_t)hart->regs[rs1(inst)] + (int32_t) imm_I(inst));
	print_d(hart,"{0x%.8X} [ADDIW] rs1: %d; rd: %d; imm: int %d uint %u",hart->pc,rs1(inst),rd(inst),(int64_t) imm_I(inst), imm_I(inst));
}
void exec_SLLIW(struct HART *hart, uint32_t inst) {
    hart->regs[rd(inst)] = (int32_t) ((int32_t)hart->regs[rs1(inst)] << (uint32_t)shamt(inst));
	print_d(hart,"{0x%.8X} [SLLIW] rs1: %d; rd: %d; imm: int %d uint %u",hart->pc,rs1(inst),rd(inst),(int64_t) shamt(inst), shamt(inst));
}
void exec_SRLIW(struct HART *hart, uint32_t inst) {
    hart->regs[rd(inst)] = (int32_t) ((uint32_t)hart->regs[rs1(inst)] >> (uint32_t)shamt(inst));
	print_d(hart,"{0x%.8X} [SRLIW] rs1: %d; rd: %d; imm: int %d uint %u",hart->pc,rs1(inst),rd(inst),(int64_t) shamt(inst), shamt(inst));
}
void exec_SRAIW(struct HART *hart, uint32_t inst) {
    hart->regs[rd(inst)] = (int64_t) ((int32_t)hart->regs[rs1(inst)] >> (uint32_t)shamt(inst));
	print_d(hart,"{0x%.8X} [SRAIW] rs1: %d; rd: %d; imm: int %d uint %u",hart->pc,rs1(inst),rd(inst),(int64_t) shamt(inst), shamt(inst));
}
...
void exec_LD(struct HART *hart, uint32_t inst) {
	...
	print_d(hart,"{0x%.8X} [LD] rs1: %d; rd: %d; imm: int %d uint %u",hart->pc,rs1(inst),rd(inst),(int64_t) imm_I(inst), imm_I(inst));
}
void exec_LWU(struct HART *hart, uint32_t inst) {
	...
	print_d(hart,"{0x%.8X} [LWU] rs1: %d; rd: %d; imm: int %d uint %u",hart->pc,rs1(inst),rd(inst),(int64_t) imm_I(inst), imm_I(inst));
}
...
void exec_SD(struct HART *hart, uint32_t inst) {
	...
	print_d(hart,"{0x%.8X} [SD] rs1: %d; rs2: %d; imm: int %d uint %u",hart->pc,rs1(inst),rs2(inst),(int64_t) imm_S(inst), imm_S(inst));
}