...
void exec_C_BEQZ(struct HART *hart, uint32_t inst) {
    ...
    uint64_t imm = (get_bits(inst, 12, 12) << 8) |
        (get_bits(inst, 6, 5) << 6)   |
...
void exec_C_ADDI4SPN(struct HART *hart, uint32_t inst) {
    ...
    uint64_t imm = (get_bits(inst, 6, 6) << 4) | (get_bits(inst, 2, 2) << 5) | (get_bits(inst, 5, 5) << 6) | (get_bits(inst, 3, 3) << 7) | (get_bits(inst, 4, 4) << 8) | (get_bits(inst, 12, 12) << 9);
