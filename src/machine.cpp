...
void machine_load_fdt(Machine& cpu, const string file_dtb = "", const string dtb_dump_path = "") {
	uint64_t dtb_path_in_memory = DRAM_BASE + cpu.memsize - 0x20000;
    if(file_dtb.length() == 0) {
        size_t dtb_size = fdt_size(cpu.fdt);
		void* buffer = malloc(dtb_size);

		size_t size = fdt_serialize(cpu.fdt,buffer,0x1000,0);
...
void machine_create_fdt(Machine& cpu, const string cmdline_append = "") {
	// Generate FDT
	fdt_node* fdt = fdt_node_create(NULL);
	...
	for(uint32_t i=0; i < cpu.core_count; i++) {
		...
		fdt_node_add_prop(cpu, "riscv,isa", "rv64ima_zicsr_zifencei_zba_zbb_zbc_zbs_sstc\0", 44);
