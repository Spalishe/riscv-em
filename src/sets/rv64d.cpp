...
FPRValue custom_min64(FPRValue t1, FPRValue t2) {
    if(isnan(t1.val) && isnan(t2.val)) {
        return std::bit_cast<double>(F_QNAN64);
    }
    else if(isnan(t1.val)) {
        if(t1.is_signaling_nan) std::feraiseexcept(FE_INVALID);
        return t2;
    }
    else if(isnan(t2.val)) {
        if(t2.is_signaling_nan) std::feraiseexcept(FE_INVALID);
        return t1;
    }
    else {
        if(!std::signbit(t1.val) && std::signbit(t2.val)) {
            return t2;
        } else if(std::signbit(t1.val) && !std::signbit(t2.val)) {
            return t1;
        } else {
...
FPRValue custom_max64(FPRValue t1, FPRValue t2) {
    if(isnan(t1.val) && isnan(t2.val)) {
        return std::bit_cast<double>(F_QNAN64);
    }
    else if(isnan(t1.val)) {
...
FPRValue accurate_round64(FPRValue val, RoundingMode rm, RoundingMode frm) {
    switch(rm) {
        case RoundingMode::RNE: return std::nearbyint(val.val);
        case RoundingMode::RTZ: return std::trunc(val.val);
        case RoundingMode::RDN: return std::floor(val.val);
        case RoundingMode::RUP: return std::ceil(val.val);
        case RoundingMode::RMM: {
            double i = std::floor(val.val);
            double f = val.val - i;
            if (f < 0.5) return i;
            if (f > 0.5) return i + 1.0;
            return (val.val >= 0) ? (i + 1.0) : i;
        }
        case RoundingMode::DYN: return accurate_round64(val,frm,frm);
    }
    return val;
}
...
inst_ret exec_FSD(HART *hart, inst_data& inst) {
    uint64_t addr = hart->GPR[inst.rs1] + (int64_t) inst.imm;
    return hart->mmio->store(hart,addr,64,std::bit_cast<uint64_t>(hart->FPR[inst.rs2].val));
}

inst_ret exec_FADD_D(HART *hart, inst_data& inst) {
    std::feclearexcept(FE_ALL_EXCEPT);
    hart->FPR[inst.rd] = update_fflags(hart,(hart->FPR[inst.rs1].val + hart->FPR[inst.rs2].val));
    return true;
}
inst_ret exec_FSUB_D(HART *hart, inst_data& inst) {
    std::feclearexcept(FE_ALL_EXCEPT);
    double res = (hart->FPR[inst.rs1].val - hart->FPR[inst.rs2].val);
    if(isinf(hart->FPR[inst.rs1].val) && isinf(hart->FPR[inst.rs2].val)) {res = std::bit_cast<double>(F_QNAN64); std::feraiseexcept(FE_INVALID);}
...
inst_ret exec_FSQRT_D(HART *hart, inst_data& inst) {
    ...
    if(hart->FPR[inst.rs1].val < 0) {
        ...
        hart->FPR[inst.rd] = update_fflags(hart,std::bit_cast<double>(F_QNAN64));
    }
    else hart->FPR[inst.rd] = update_fflags(hart,sqrt(hart->FPR[inst.rs1].val));
    return true;
}

inst_ret exec_FMIN_D(HART *hart, inst_data& inst) {
    std::feclearexcept(FE_ALL_EXCEPT);
    FPRValue res = custom_min64(hart->FPR[inst.rs1],hart->FPR[inst.rs2]);
    hart->FPR[inst.rd] = update_fflags(hart,res);
    return true;
}
inst_ret exec_FMAX_D(HART *hart, inst_data& inst) {
    std::feclearexcept(FE_ALL_EXCEPT);
    hart->FPR[inst.rd] = update_fflags(hart,custom_max64(hart->FPR[inst.rs1],hart->FPR[inst.rs2]));
    return true;
}

inst_ret exec_FMADD_D(HART *hart, inst_data& inst) {
    std::feclearexcept(FE_ALL_EXCEPT);
    hart->FPR[inst.rd] = update_fflags(hart,((hart->FPR[inst.rs1].val * hart->FPR[inst.rs2].val) + hart->FPR[inst.rs3].val));
    return true;
}
...
inst_ret exec_FCVT_W_D(HART *hart, inst_data& inst) {
    ...
    hart->GPR[inst.rd] = (uint64_t)(uint64_t)saturate_round<int32_t>(update_fflags(hart,hart->FPR[inst.rs1],inst));
    return true;
}
inst_ret exec_FCVT_WU_D(HART *hart, inst_data& inst) {
    ...
    hart->GPR[inst.rd] = (uint64_t)(int32_t)saturate_round<uint32_t>(update_fflags(hart,hart->FPR[inst.rs1],inst));
    return true;
}
inst_ret exec_FCVT_L_D(HART *hart, inst_data& inst) {
    ...
    hart->GPR[inst.rd] = (uint64_t)saturate_round<int64_t>(update_fflags(hart,hart->FPR[inst.rs1],inst));
    return true;
}
inst_ret exec_FCVT_LU_D(HART *hart, inst_data& inst) {
    ...
    hart->GPR[inst.rd] = (uint64_t)saturate_round<uint64_t>(update_fflags(hart,hart->FPR[inst.rs1],inst));
    return true;
}

inst_ret exec_FCVT_D_W(HART *hart, inst_data& inst) {
    std::feclearexcept(FE_ALL_EXCEPT);
    hart->FPR[inst.rd] = update_fflags(hart,(int32_t)hart->GPR[inst.rs1],inst);
    return true;
}
inst_ret exec_FCVT_D_WU(HART *hart, inst_data& inst) {
    std::feclearexcept(FE_ALL_EXCEPT);
    hart->FPR[inst.rd] = update_fflags(hart,(uint32_t)hart->GPR[inst.rs1],inst);
    return true;
}
inst_ret exec_FCVT_D_L(HART *hart, inst_data& inst) {
    std::feclearexcept(FE_ALL_EXCEPT);
    hart->FPR[inst.rd] = update_fflags(hart,(int64_t)hart->GPR[inst.rs1],inst);
    return true;
}
inst_ret exec_FCVT_D_LU(HART *hart, inst_data& inst) {
    std::feclearexcept(FE_ALL_EXCEPT);
    hart->FPR[inst.rd] = update_fflags(hart,(uint64_t)hart->GPR[inst.rs1],inst);
    return true;
}

inst_ret exec_FSGNJ_D(HART *hart, inst_data& inst) {
    std::feclearexcept(FE_ALL_EXCEPT);
    hart->FPR[inst.rd] = update_fflags(hart,(double)copysign(hart->FPR[inst.rs1].val,hart->FPR[inst.rs2].val));
    return true;
}
...
inst_ret exec_FSGNJX_D(HART *hart, inst_data& inst) {
    std::feclearexcept(FE_ALL_EXCEPT);
    uint64_t sign1 = std::bit_cast<uint64_t>(hart->FPR[inst.rs1].val) & 0x8000000000000000;
    uint64_t sign2 = std::bit_cast<uint64_t>(hart->FPR[inst.rs2].val) & 0x8000000000000000;
    uint64_t other = std::bit_cast<uint64_t>(hart->FPR[inst.rs1].val) & 0x7fffffffffffffff;
    hart->FPR[inst.rd] = update_fflags(hart,std::bit_cast<double>(((sign1 ^ sign2) | other)));
    return true;
}

inst_ret exec_FMV_X_D(HART *hart, inst_data& inst) {
    std::feclearexcept(FE_ALL_EXCEPT);
    hart->GPR[inst.rd] = std::bit_cast<uint64_t>(hart->FPR[inst.rs1].val);
    return true;
}
inst_ret exec_FMV_D_X(HART *hart, inst_data& inst) {
    std::feclearexcept(FE_ALL_EXCEPT);
    hart->FPR[inst.rd] = update_fflags(hart,std::bit_cast<double>((uint64_t)hart->GPR[inst.rs1]));
    return true;
}
