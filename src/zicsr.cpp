...
void exec_CSRRW(struct HART *hart, uint32_t inst) {
	...
	print_d(hart,"{0x%.8X} [CSRRW] rs1: %d; rd: %d; imm: int %d uint %u",hart->pc,rs1(inst),rd(inst),(int64_t) imm_Zicsr(inst), imm_Zicsr(inst));
}
void exec_CSRRS(struct HART *hart, uint32_t inst) {
	...
	print_d(hart,"{0x%.8X} [CSRRS] rs1: %d; rd: %d; imm: int %d uint %u",hart->pc,rs1(inst),rd(inst),(int64_t) imm_Zicsr(inst), imm_Zicsr(inst));
}
void exec_CSRRC(struct HART *hart, uint32_t inst) {
    ...
    hart->regs[rd(inst)] = init_val;
	print_d(hart,"{0x%.8X} [CSRRC] rs1: %d; rd: %d; imm: int %d uint %u",hart->pc,rs1(inst),rd(inst),(int64_t) imm_Zicsr(inst), imm_Zicsr(inst));
}
...
void exec_CSRRWI(struct HART *hart, uint32_t inst) {
	...
	print_d(hart,"{0x%.8X} [CSRRWI] rs1: %d; rd: %d; imm: int %d uint %u",hart->pc,rs1(inst),rd(inst),(int64_t) imm_Zicsr(inst), imm_Zicsr(inst));
}
void exec_CSRRSI(struct HART *hart, uint32_t inst) {
    ...
    hart->regs[rd(inst)] = init_val;
	print_d(hart,"{0x%.8X} [CSRRSI] rs1: %d; rd: %d; imm: int %d uint %u",hart->pc,rs1(inst),rd(inst),(int64_t) imm_Zicsr(inst), imm_Zicsr(inst));
}
void exec_CSRRCI(struct HART *hart, uint32_t inst) {
    ...
    hart->regs[rd(inst)] = init_val;
	print_d(hart,"{0x%.8X} [CSRRCI] rs1: %d; rd: %d; imm: int %d uint %u",hart->pc,rs1(inst),rd(inst),(int64_t) imm_Zicsr(inst), imm_Zicsr(inst));
}